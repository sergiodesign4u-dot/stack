...
| Wireframes | **In progress** (`wireframes/` — greyscale clickable prototype). **Флоу 1 (beginner) COMPLETE + critiqued (Step 9)**: 38 html — home(+buyer/coach, IG-stories promo) · listing(+5 states) · goal · product(+4 states) · cart · checkout(+loggedin/noaddr/loading/declined) · auth(+code/loading/newuser/error) · order-placed · account, all states built. Step 9 synced a cart-desync to one canonical basket (guest 3 999 ₴ / logged-in 3 802 ₴), added the missing matrix states, wired the payment happy path, and logged everything in `wireframes/_critique.md`. Flow-first nav (`index.html` «Всі екрани» + `_nav.js` coverage source of truth + prototype bar). Next: Step 8 fan-out the rest (coach/reorder/loyalty/content/system) via subagents. Method: `playbook/design-wireframes-playbook.md`, contract `wireframes/_conventions.md`, matrix `wireframes/_screens.md`, critique `wireframes/_critique.md`. |
| Concept | Not started |
