...
| **width sweep re-run at 12.7** | **done at 12.7** | the same instrument over 124 coloured pages instead of 91, and it was not a repeat run: **two more wrong versions surfaced, and both were showing as a clean zero.** A clipping ancestor ended the search for a rail, so `SPAN.hptag` inside `.hpromo` inside `.hslider` was reported as cut off by 488px on the four home screens; and a record was filed by its FIRST crossing, so an owner crossing at 324 and again at 374, 424, 474, 524 and 574 went to the below-floor block while the class counter above it read `0`. Ten phantom rows left, four real ones arrived. The five genuine findings that remained were ONE cause - `--container-text` is `68ch`, and `ch` belongs to the font it is read against, while the ceiling sat on `.info-body` at 16px and the prose inside it is drawn at 14 - repaired in `info-page.css` and re-checked by the same instrument: **0 above the floor on every one of 129 widths, all 124 pages** |
