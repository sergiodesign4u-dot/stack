...
## Entry points
Home hero «Підібрати за квізом» (4.0); optionally goal tiles → start quiz with Q1 preset; a banner
inside the Goal collection. Exit «Просто відкрити каталог» → 2.0 (no pressure).
...
**Open [?]:** 4 vs 5 questions; start quiz from a goal tile (Q1 preset)?; email-save of the set; exact
set-curation rules (data/content); whether the quiz appears in global nav or only on Home.
