...
- **Page-level specs:** one md source of truth per node in `ia/docs/pages/`, each rendered
  ...
  - `pages/coach.md` → `ia/coach.html` (node 5.x — **Coach workspace, PRIMARY Job 1**, whole cluster in
    ...
    (saved clients + tag-to-client + per-client history) · **5.4a Edit client** (edit modal: name/goal/
    notes + delete behind a confirm; states base/confirm) · **5.5 Multi-client order session** ★ (client
...
- **5 — Coach workspace (PRIMARY, Decision 1):** 5.0 For-Coaches landing + published
  ...
  5.4 Client profile (+ 5.4a Edit client) · 5.5 Multi-client order session · 5.6 Order history ·
