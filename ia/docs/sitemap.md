...
- **Page-level specs:** one md source of truth per node in `ia/docs/pages/`, each rendered
  ...
  - `pages/catalog.md` → `ia/catalog.html` (cluster 2 — the **store taxonomy**: 14 top
    categories → subcategories → types, goals as cross-cutting collections, facets; built
    from real UA catalogs, Belok-based).
