# Wireframes - Screen × State matrix (`docs/screens.md`)
...
**Next step → Step 2:** write `wireframes/docs/conventions.md` (the contract), inheriting the existing
...
Переприв'язав обидва входи в `coach-home` (сайдбар «Тариф» + «Керувати тарифом»).
...
**index.html - рейка категорій = flyout ОВЕРЛЕЙ.** Винесено `wfHomeRail()` у `_nav.js` + спільні
`.hrail-fly`/`.hrail-scrim` у `_wf.css`; index.html відкриває flyout по ховеру над scrim; `home-catalog.html`
