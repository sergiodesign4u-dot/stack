# Wireframes - Conventions / the contract (`docs/conventions.md`)
...
screen + its states + click-wiring - then **Флоу 2 (Тренер)**, with `overview.html` tracking coverage
...
4. **[9] Critique** - defect table → fix (dead ends + missing states first) → `docs/critique.md`.
...
- The **product card** is the canonical one from the IA (`index.html`) - same markup/class everywhere;
...
- **File naming (Latin, from the sitemap):** base screen `wireframes/<name>.html` (= the default/
  ...
  - Role/domain states get an explicit suffix: `-guest` · `-coach` · `-oos` · `-noaddr` ·
    ...
    marked for the screen in `docs/screens.md`.** Don't invent a state the matrix doesn't list.
- **Canonical name map** (from `docs/screens.md`, main flow):
...
4. **Step 6 - rest of the main flow:** `goal`, `product`, `cart`, `checkout`, `auth`,
   `order-placed`, `account` + each one's states from `docs/screens.md`.
...
7. **Step 9 - review** → `docs/critique.md` → fix → update CLAUDE.md/README → push.
