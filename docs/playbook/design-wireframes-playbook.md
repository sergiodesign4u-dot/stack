...
>   `wireframes/docs/screens.md` and `wireframes/docs/conventions.md`.
...
1. **Choose screens** - read `sitemap.md`, `flows.md`, `jtbd.md`. List the **main-flow** screens
   ...
   Build the `wireframes/docs/screens.md` matrix. No screen without a job and a flow position.
2. **Write the rules** - `wireframes/docs/conventions.md`, inheriting the existing CSS/tokens/sidebar/
...
9. **Review & finalize** - a scrupulous pass vs `_conventions.md`, `sitemap.md`, `flows.md`. First
   ...
   `wireframes/docs/critique.md`. Finally update CLAUDE.md ("Wireframes") + README.md ("Wireframes")
   and push.
...
- **2026-07-01** - **Step 9 (critique) done for Флоу 1** + a promo-banner rework. Ran the critique as a
  ...
  **before touching anything** (the method's "table first" checkpoint), then fixed dead-ends/missing-states
  first. Fixes (full log → `wireframes/docs/critique.md`): **H1** the known cart-desync - locked ONE canonical
...
- **Milestone - Флоу 1 + Флоу 2 COMPLETE + CRITIQUED (2026-07-02):** ~64 html; both flows internally
  consistent; coverage clean; critique log in `wireframes/docs/critique.md`.
